...
static bool gesture_battery_charging_usb = 0;
static bool gesture_battery_charging_ac = 0;

bool get_charging()
{
    return ((gesture_battery_charging_ac) || (gesture_battery_charging_usb));
}
...
static void ac_update(struct ac_data *ac_data)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		#endif
			ac_data->AC_ONLINE = 1;
			gesture_battery_charging_ac = 1;
			ac_psy->type = POWER_SUPPLY_TYPE_MAINS;
		} else {
			ac_data->AC_ONLINE = 0;
			gesture_battery_charging_ac = 0;
		}
	} else {
		ac_data->AC_ONLINE = 0;
    	gesture_battery_charging_ac = 0;		
...
static void usb_update(struct usb_data *usb_data)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		if ((BMT_status.charger_type == STANDARD_HOST) ||
			...
			gesture_battery_charging_usb = 1;
			usb_psy->type = POWER_SUPPLY_TYPE_USB;
		} else {
			usb_data->USB_ONLINE = 0;
    		gesture_battery_charging_usb = 0;
		}
	} else {
		usb_data->USB_ONLINE = 0;
		gesture_battery_charging_usb = 0;
