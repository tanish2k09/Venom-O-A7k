 ...
 */
static bool disable_lineageos_access = 0;
 
static ssize_t mtk_disp_ld_set_rgb(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{

    if (disable_lineageos_access)
        return count;
...
void force_livedisplay_set_rgb(int force_r, int force_g, int force_b)
{
    ...
    cancel_work_sync(&mtk_rgb_work_queue.work);
	mtk_disp_ld_r = force_r;
	mtk_disp_ld_g = force_g;
	mtk_disp_ld_b = force_b;
	schedule_work(&mtk_rgb_work_queue.work);
...
#ifdef CONFIG_MTK_VIDEOX_CYNGN_LIVEDISPLAY
void update_disp_mgr(bool force_l)
{
    disable_lineageos_access = force_l;
