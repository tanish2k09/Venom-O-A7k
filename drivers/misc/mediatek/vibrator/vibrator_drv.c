...
bool vib_scr_suspended = false;
static bool local_vibration_override=false;
static unsigned int haptic_factor = 100;
...
{
    if (vibrator_on_lock==false)
    {
        if((no_vibrate==false)||(local_vibration_override==true))
        {
	        if (!ldo_state) {
		        vibr_Enable_HW();
		        ldo_state = 1;
...
/*lenovo-sw lixh10 add for LENOVO_VIBRATOR_HAPTIC_FEEDBACK*/
		...
		hrtimer_start(&vibe_timer,
			      ktime_set(value / 1000, (value % 1000) * (100000000/haptic_factor)), HRTIMER_MODE_REL);
	}
...
{
    local_vibration_override=true;
    vibr_Enable();
 	vibrator_enable(&mtk_vibrator, value);
    local_vibration_override=false;
}
...
static ssize_t show_haptic_factor(struct device *dev, struct device_attribute *attr, const char *buf,
			     size_t size)
{
	size_t count = 0;
...
static ssize_t store_haptic_factor(struct device *dev, struct device_attribute *attr, const char *buf,
			     size_t size)
{
	unsigned int haptic = haptic_factor;
	...
	if (buf != NULL && size != 0) {
		...
		if(haptic < 100)
			...
			haptic = 1000;
		haptic_factor = haptic;
	}
	
	return size;
}

static DEVICE_ATTR(haptic_factor, 0666, show_haptic_factor, store_haptic_factor);
...
static int vib_mod_init(void)
{
	...
	ret = device_create_file(mtk_vibrator.dev, &dev_attr_haptic_factor);
	if (ret) {
		printk("[vibrator]device_create_file haptic_factor fail!\n");
	}
