...
static struct opp_tbl_info opp_tbls[] = {
    ...
    #ifdef CONFIG_MTK_CPU_OC
    [CPU_LV_TO_OPP_IDX(CPU_LEVEL_2)] = { ARRAY_AND_SIZE(opp_tbl_e1_0), },
    #else
...
#define PLL_MAX_FREQ		(2002000)	/* KHz */ // TODO: check max freq
...
static int mtk_cpunum_throttle_num = 6;
static int mtk_cpunum_throttle_battery_limit = 20;
...
ssize_t store_UV_mV_table(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	...
	&u[0], &u[1], &u[2], &u[3], &u[4], &u[5], &u[6], &u[7], &u[8], &u[9], &u[10],&u[11]);	// in MHz
