...
static bool force_livedisplay, target_achieved, brightness_lvl_auto_enable;
...
static unsigned int brightness_lvl, b_cache;
static unsigned int active_minutes, livedisplay_aggression;
static int livedisplay_start_hour, livedisplay_stop_hour, brightness_lvl_auto_hour_start, brightness_lvl_auto_hour_end;
extern void update_disp_mgr(bool force_l);

//////////////////////////////////////

extern volatile int wlan_if_changed;
extern INT8 wlan_debug_local_msg;
...
inline static void calc_active_minutes()
{
    active_minutes = (24 + livedisplay_stop_hour - livedisplay_start_hour)*60;
}

inline static int get_minutes_since_start()
{
...
inline static int get_minutes_before_stop()
{
    int min = (active_minutes - get_minutes_since_start());
    return min;
}

inline static void force_livedisplay_set_rgb_brightness(int r,int g,int b)
{
	...
	if (suspend) {
		...
		this_thunderx->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
            ...
            if (force_livedisplay == 1)
            {
	            ...
	            //First we check for brightness automation...
	            if (brightness_lvl_auto_enable == 1)
	            {
	            if(!((tm.tm_hour >= brightness_lvl_auto_hour_start) || (tm.tm_hour < brightness_lvl_auto_hour_end))) //Means not in b-time
	                brightness_lvl = 10;
	            else
...
inline static unsigned int validate_freq(struct cpufreq_policy *policy, int freq) {
	...
	if (do_boost)
		freq = 1690000; //Handle gaming mode freq scaling. Bool gaming_mode will switch according to suspend state before coming to this function.
						//due to the fact that it is set in update_min_max function :D
...
static ssize_t show_wlan_cfg(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%d %s\n", wlan_if_changed,wlan_debug_local_msg);
}

static ssize_t store_wlan_cfg(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{	
	return count;
}
...
define_global_rw_attr(wlan_cfg);
...
// ********* Venom kernel additions for A7000 (aio_row) inits **********
    ...
    pocket_mode_dt2w_override_taps_number = 4;
    ...
    brightness_lvl_auto_hour_end = 5;
